...
namespace millikan {
	int get_ohm(){
		cerr << "Calcolo resistenza al variare del tempo" << endl;
		double T=0;
		cerr << "Temperatura: ";
		cin >> T;
	...
	long double velocita_regime(long double r ){
		return 2*G*pow(r,2)*(uOlio-uAria)/(9*nAria);
	}
	long double forza_viscosa(long double r,long double v){
		return 6*M_PI*r*nAria*v;
	}
	long double forza_peso(long double r,long double v){
